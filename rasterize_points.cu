...
RasterizeGaussiansCUDA(
    ...
    const torch::Tensor& projmatrix_raw, // Added
	...
	const bool debug)
{
  ...
  torch::Tensor n_touched = torch::full({P}, 0, means3D.options().dtype(torch::kInt32)); // Added
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		n_touched.contiguous().data<int>(), // Added
		debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, radii, geomBuffer, binningBuffer, imgBuffer, proj_2D, conic_2D, conic_2D_inv, gs_per_pixel, weight_per_gs_pixel, x_mu, n_touched); // added n_touched
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dtau = torch::zeros({P,6}, means3D.options()); // Added
  ...
  {  
	  ...
	  viewmatrix.contiguous().data<float>(),
	  projmatrix.contiguous().data<float>(),
      projmatrix_raw.contiguous().data<float>(), // Added
	  campos.contiguous().data<float>(),
	  tan_fovx,
	  tan_fovy,
	  radii.contiguous().data<int>(),
      ...
      dL_dtau.contiguous().data<float>(), // Added
	  debug);
  }

  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dtau); // Added dL_dtau
}
